...
// Return encodes which limit was hit: steps if target limit hit, -(steps) if max_steps reached.
// The result also encodes the hit limit in the high bits via a packed format:
//   result >= 0: steps taken, hit the target limit
//   result == -3: max steps reached without hitting limit
//   result == -1: invalid motor_id
//   result == -2: motor not initialized
int rpc_move_until_limit(int motor_id, int direction, int delay_us, int max_steps) {
  ...
  // If not at any limit, check whichever limit is in our travel direction
  bool checkForMin = !startedAtMin;
  bool checkForMax = !startedAtMax;

  int steps = 0;
  int checkInterval = 50;  // Check limits every N steps (pause briefly to avoid EMI)
